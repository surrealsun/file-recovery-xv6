...
{	
	...
	begin_op();
	if((ip = namei(path)) == 0){ //Returns the inode located at the end of the path.
			end_op();
			return -1;
	}
	ilock(ip);
	if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){ // filealloc() is located in file.c, fdalloc(f) is in the current file.
		if(f)
			fileclose(f);
		iunlockput(ip);
		end_op();
	...
	int del_count=0, i=0, j=0; 								
	while(fileread(f, &de, sizeof(de)) == sizeof(de)){
		if(de.del == 1){
			char *name;
			strncpy(name, de.name, DIRSIZ);
			while(j<strlen(name)){
				result[i+j]=name[j];		//i - row number; j - offset in the i-th row
				j++;			
			}
			result[i+j]=0;
			if(del_count==64) 
				break;
			i = (++del_count)*(DIRSIZ+1);			
			j=0;		
		}			
	}
...
sys_rec(void){
	...
	//Is the path to the parent directory valid?
	if((dp = nameiparent(path, name)) == 0) {// fs.c 
		end_op();	
		return -1;
	}
	ilock(dp);
	// Is the deleted file located in the directory?	
	if((ip = dirdeletedlookup(dp, name, &off)) == 0){ // fs.c
		iunlockput(dp);
		end_op();
		return -2;
	}
	// Is the inode used for something else? (ALSO LOCKS THE INODE)
	...
	// Is any data block of the inode used for something else?
	bn = ((ip->size-1)/512 + 1);
	if((checkibloks(ip,bn)) == 0){ // fs.c
		
		// Set unrecoverable (the file can no longer be recovered; the next time rec is called for this file, it will return -1)
		memset(&de, 0, sizeof(de));
		if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de)) // fs.c
			panic("unlink: writei");
	...
	// Is there a file in the directory that has the same name as the file we are trying to recover?
	if(dirlookup(dp,name,0) > 0){
		int k = strlen(name)-1;
		char c = (name[k]+1 > 122) ? 'a' : name[k] + 1;
		
		if(k+1 < DIRSIZ)
			name[k + 1] = c; //We are adding a character.
		else
			name[k] = c; // We are changing the last character of the name.
	}
	...
	if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de)) //fs.c; we are writing changes to the disk.
		panic("sys_rec: writei");
		
	ip->nlink++;
	iupdate(ip); // We are writing changes to the disk.
